...
class ZradAid_Jstree_Struct
{
    ...
    function __construct($table = "tree", $fields = array())
    {
        ...
        // Database
        $this->db = new ZradAid_Jstree_Database();
    }
    ...
    function _get_children($id, $recursive = false)
    {
        ...
        if ($recursive) {
            ...
            $this->db->query("SELECT `" . implode("` , `", $this->fields) . "` FROM `" . $this->table . "` WHERE `" . $this->fields["left"] . "` >= " . (int) $node[$this->fields["left"]] . " AND `" . $this->fields["right"] . "` <= " . (int) $node[$this->fields["right"]] . " AND es_activo != 0 ORDER BY `" . $this->fields["left"] . "` ASC");
        } else {
            $this->db->query("SELECT `" . implode("` , `", $this->fields) . "` FROM `" . $this->table . "` WHERE `" . $this->fields["parent_id"] . "` = " . (int) $id . " AND es_activo != 0 ORDER BY `" . $this->fields["position"] . "` ASC");
        }
        while ($this->db->nextr())
